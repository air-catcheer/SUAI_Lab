...
Matrix::Matrix(vector<vector<double>> temp) : Matrix(get_size(temp))
{
    arr = temp;
}
...
Matrix &Matrix::operator*=(const Matrix &b)
{
    ...
    for (int result_x = 0; result_x < size; result_x++)
    {
        for (int result_y = 0; result_y < size; result_y++)
        {
            double summ = 0;
            for (int i = 0; i < size; i++)
                summ += arr[result_y][i] * b.arr[i][result_x];
...
Matrix &Matrix::operator/=(const Matrix &b)
{
    // дописать инвертирование
    // спросить про рекурсивное нахождение определителя
    *this *= inverse(b);
    return *this;
}
